...
void edge(int width, int height, float* suppression, float* hyst, float* edges) {
	int t_high, t_low, i, j, sum, percent = (0.9*width*height);
...
int main (int argc, char** argv){
	...
	***************/
	...
	MPI_Gather(edges, (width*height)/num_proc, MPI_FLOAT, new_edges, (width*height)/num_proc, MPI_FLOAT, 0, MPI_COMM_WORLD);
