...
/*
TERM NOTES:
BLOCK refers to the section of the CUDA CORE.
THREAD refers to the individual thread within that CUDA CORE.
y is the columns
x is the rows
*/
__global__
void hconvolve_L2cache(float *in_image,int width,int height,float *mask,int mask_width,int mask_height,float *out_image){
  __shared__ float* Ashared;
  //float* temp_tile;
  int k = 0;
  float sum;
  //temp_tile = (float*) malloc(blockDim.x*blockDim.y);
  Ashared = (float*) malloc(blockDim.x*blockDim.y);
  int offsetj;
  int i=blockIdx.x*blockDim.x + threadIdx.x;
  int j=blockIdx.y*blockDim.y + threadIdx.y;

  //Copy all pixels of the assigned section to this tile.
  for (i; i < i+blockDim.x ; i++){
      for(j; j < j+blockDim.y; j++)
         Ashared[threadIdx.x*blockDim.x + threadIdx.y + j] = in_image[i*width+j];
  }

  __syncthreads();

  
  //This is horizontal convolution
  for(k = 0; k < mask_width; k++) {
	offsetj = -1*floorf(mask_width/2)+k;
	if (threadIdx.y + offsetj < blockDim.y && threadIdx.y + offsetj >= 0){
		sum+= Ashared[(threadIdx.x*blockDim.y)+(threadIdx.y+offsetj)]*mask[k];
		printf("this\n");
	}
	else if((blockIdx.y*blockDim.y+threadIdx.y+offsetj) >= 0 && (blockIdx.y*blockDim.y+threadIdx.y+offsetj) < width) {
		sum += in_image[i*width+(blockIdx.y*blockDim.y+threadIdx.y+offsetj)]*mask[k];
		printf("that\n");
	}

	out_image[i*width+(blockIdx.y*blockDim.y+threadIdx.y+offsetj)]=(float)sum; 
  }
  __syncthreads();
...
/*
__global__
void vconvolve_L2cache(float *in_image,int width,int height,float *mask,int mask_width,int mask_height,float *out_image){
  __shared__ float* Ashared;
  //float* temp_tile;
  int k = 0;
  float sum;
  ...
  //This is vertical convolution
  for(k = 0; k < mask_width; k++) {
	offseti = -1*floorf(mask_width/2)+k;
	if (threadIdx.x + offseti < blockDim.x && threadIdx.x + offseti >= 0)
		sum+= Ashared[(threadIdx.x*blockDim.y)+(threadIdx.y+offseti)]*mask[k];
	else if((blockIdx.x*blockDim.x+threadIdx.y+offsetj) >= 0 && (blockIdx.y*blockDim.y+threadIdx.y+offsetj) < width)
			sum += in_image[i*width+(blockIdx.x*blockDim.x+threadIdx.y+offseti)]*mask[k];

	out_image[i*width+(blockIdx.x*blockDim.x+threadIdx.y+offseti)]=(float)sum; 
  }
...
int block_dim=16;
 ...
 dim3 dimBlock(block_dim,block_dim,1);
  ...
  hconvolve_L2cache<<<dimGrid,dimBlock>>>(d_org_img,width,height,d_gaussian_kernel,1,k_width,d_temp_horizontal); 
  hconvolve_L2cache<<<dimGrid,dimBlock>>>(d_temp_horizontal,width,height,d_gaussian_deriv,k_width,1,d_horizontal_gradient);
/*
  //Vertical gradient. horizontal kernel then vertical derivative
  vconvolve_L2cache<<<dimGrid,dimBlock>>>(d_org_img,width,height,d_gaussian_kernel,k_width,1,d_temp_vertical); 
  vconvolve_L2cache<<<dimGrid,dimBlock>>>(d_temp_vertical,width,height,d_gaussian_deriv,1,k_width,d_vertical_gradient);
 */
